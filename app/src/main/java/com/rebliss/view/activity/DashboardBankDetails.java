...
public class DashboardBankDetails extends AppCompatActivity {
    ...
    //commented to not show optional as it is now mandatory......
    private void setHint() {
        textAcName.setText(ShowHintOrText.GetMandatory("Bank Account Holder Name"));
        textBankName.setText(ShowHintOrText.GetMandatory("Bank name"));
        textAcNo.setText(ShowHintOrText.GetMandatory("Account No."));
        textcAcNo.setText(ShowHintOrText.GetMandatory("Confirm account No."));
        textIFSC.setText(ShowHintOrText.GetMandatory("IFSC Code"));
    }
    ...
    private boolean isFormValidated() {
        ...
        if (sAcName.length() <= 0) {
            status = false;
            showWarningSimpleAlertDialog(Constant.TITLE, getString(R.string.ACCOUNT_NAME));
        }

        if (sAcNo.length() <= 0) {
            status = false;
            showWarningSimpleAlertDialog(Constant.TITLE, getString(R.string.ACCOUNT_NUMBER));
        }

        if (scAcNo.length() <= 0) {
            status = false;
            showWarningSimpleAlertDialog(Constant.TITLE, getString(R.string.C_ACCOUNT_NUMBER));
        }

        if (sIFSC.length() <= 0) {
            status = false;
            showWarningSimpleAlertDialog(Constant.TITLE, getString(R.string.IFSC_NUMBER));
        }
        if (!sAcNo.equals(scAcNo)) {
            status = false;
            showWarningSimpleAlertDialog(Constant.TITLE, getString(R.string.VALID_ACCOUNT));
        }
        return status;
    }
